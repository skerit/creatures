...
Export.setMethod(function processBuffer(buffer) {
	...
	// Next bit is always 0x0200
	ex.skip(2);

	// Create the chemicals array
	this.chemicals = [];

	// Now read in all the chemicals, these are [Value] [Half life]
	for (i = 0; i < Blast.Classes.Develry.Creatures.Creature.chemicals.length; i++) {
		// Get the name of the chemical
		name = Blast.Classes.Develry.Creatures.Creature.chemicals[i];
		...
		temp = {
			...
			name     : String(name),
			value    : ex.readByte(1),
			halflife : ex.readByte(1)
		};

		this.chemicals.push(temp);
	}

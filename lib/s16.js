 ...
 * @return   {Number}
 */
S16.setStatic(function meanReduce(values) {
		...
		for (j = 0; j < entry.buffer.length; j += 2) {
			...
			if (this.pigment_bleed_info) {
				...
				for (k = 0; k < this.pigment_bleed_info.length; k++) {
					...
					temp = S16.hslToRgb(hue, hsl[1], hsl[2]);
					...
					// Apply swap
					temp[0] = (sr + ((sr * (1-swap)) + (sb * swap))) / 2;
					temp[2] = (sb + ((sb * (1-swap)) + (sr * swap))) / 2;
				...
				// Take the reduce mean of all the new values and apply those
				entry.rgba[index  ] = S16.meanReduce(r);
				entry.rgba[index+1] = S16.meanReduce(g);
				entry.rgba[index+2] = S16.meanReduce(b);
			}
		}
	}
});

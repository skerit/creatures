 ...
 * @param    {String}                 path
 */
S16 = Fn.inherits('Informer', 'Develry.Creatures', function S16(app, path) {
 ...
 */
S16.setMethod(function load(callback) {
...
S16.setMethod(function processBuffer(buffer) {
	...
	for (i = 0; i < this.image_count; i++) {
		...
		entry.rgba = new Uint8ClampedArray(entry.width * entry.height * 3);
	}
	...
	for (i = 0; i < this.image_count; i++) {
		...
		for (j = 0; j < entry.buffer.length; j += 2) {
			...
			if (this.format == 0) { // 555
				// Red
				entry.rgba[j] = (number & 0x7c00) >> 7;

				// Green
				entry.rgba[j+1] = (number & 0x03e0) >> 2;

				// Blue
				entry.rgba[j+2] = (number & 0x001f) << 3;

				// Alpha (255 is fully visible)
				entry.rgba[j+3] = 255;
			} else { // 565
				// Red
				entry.rgba[j] = (number & 0xf800) >> 8;

				// Green
				entry.rgba[j+1] = (number & 0x070) >> 3;

				// Blue
				entry.rgba[j+2] = (number & 0x01f) << 3;

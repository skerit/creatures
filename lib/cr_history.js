...
History.setMethod(function loadCreature(callback) {
	...
	if (this.creature) {
		return Blast.nextTick(callback, null, null, this.creature);
	}
...
History.setMethod(function readCstring(buffer) {
	...
	if (length == 0xFF) {
		length = buffer.readUInt16LE(idx);
		idx += 2;
	}
...
History.setMethod(function readLong(buffer) {
	...
	if (!end) {
		throw new Error('Unable to read Long, no valid index given');
	}
...
History.setMethod(function processBuffer(buffer) {
	...
	if (!buffer || !buffer.length) {
		throw new Error('Can not process empty buffer');
	}

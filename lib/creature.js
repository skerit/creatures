...
Creature.setProperty(function moniker() {
	...
	if (moniker) {
		...
		// "Export" instances inherit from Creatures, but they should not be registerd
		if (this.in_world_type !== false) {
...
Creature.setMethod(function move(x, y, callback) {
	...
	if (isNaN(x) || isNaN(y)) {
		return Blast.nextTick(callback, null, new Error('Can not move creature to invalid coordinates'));
	}
...
Creature.setCacheMethod(function loadHistory(callback) {
	...
	this.log('verbose', 'Going to load history file of creature', this.moniker);
	...
	Fn.parallel(function getWorldName(next) {
		...
		if (err) {
			that.log('error', 'Error loading history of creature', that.moniker, ':', err);
			return callback(err);
		}
...
Creature.setMethod(function getParent(type, callback) {
	...
	if (this.hasBeenSeen('getting_' + type)) {
		...
		this.log('verbose', 'Getting', type, 'of', this.moniker, 'from cache');

		// The event name might start with "got_", it is also fired
		// when the parent is unable to be found
		return this.afterOnce(event, function gotParent() {
			callback(null, that[prop]);
		});
	}

	this.log('verbose', 'Getting', type, 'of', this.moniker, 'for first time');
	...
	function done(err, result) {
		...
		if (err) {
			...
			that.log('verbose', 'Found', type, 'of', that.moniker, ':', result.moniker);
		} else {
			that.log('debug', 'Did not find', type, 'of', that.moniker);
		}

		Blast.nextTick(function emitEvent() {
			that.emit(event);
		});
...
Creature.setMethod(function getGeneration(callback) {
	...
	if (this.hasBeenSeen('getting_generation')) {
		return this.once('_got_generation', callback);
	}
	...
	if (this.note_vars && this.note_vars.generation) {
		...
		if (!isNaN(this.generation)) {
			...
			this.emit('_got_generation', null, gen);
			return;
		}
	}
	...
	Fn.series(function getFather(next) {
		...
		that.unsee('getting_generation');

		if (err) {
			that.log('error', 'Failed to get generation:', err);
			that.emit('_got_generation', err);
			return callback(err);
		}
		...
		that.emit('_got_generation', null, gen);
	});
});
	...
	Fn.series(function doUpdate(next) {
		...
		that.update(function done(err) {
			...
			if (err) {
				...
				if (attempt > 10) {
					return next(new Error('Unable to update creature ' + that.moniker + ', tried 10 times: ' + err));
				}

				that.log('warn', 'Failed to update creature before export, making attempt ' + attempt + ' in 500ms');

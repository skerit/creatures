...
Creature.setMethod(function update(callback) {
	...
	Fn.parallel(function getName(next) {
		...
		cmd = 'dde: putv camn,dde: putv chem ' + health_chemical + ',dde: putv dead,dde: putv spcs,dde: putv cage,dde: putv gnus,dde: putv drv!,dde: putv uncs,dde: putv baby,dde: putv aslp';
		...
		that.command(cmd, function gotVitals(err, response) {
			...
			// Is it unconcious?
			that.unconcious = Boolean(Number(pieces[7]));
...
Creature.setAfterMethod('ready', function getBodyPartImage(part_name, callback) {
	this.gene.getBodyPartImage(this, part_name, callback);
});

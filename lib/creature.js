 ...
 */
Creature = Fn.inherits('Develry.Creatures.Base', function Creature(app) {
...
Creature.setMethod(function teachLanguage(callback) {
	...
	return this.command('sys: cmnd 32777', callback);
});
...
Creature.setMethod(function move(left, right, callback) {
	return this.command('mcrt ' + left + ' ' + right, callback);
});
 ...
 * @param    {Function} callback
 */
Creature.setMethod(function loadGeneFile(callback) {
	...
	this.app.getWorlds(function gotWorlds(err, worlds, history_maps) {
		...
		history_map = Blast.Bound.Object.first(history_maps);
		gene_map = libpath.resolve(history_map, '..', '..', 'Genetics');
...
Creature.setCacheMethod(function loadHistoryFiles(callback) {
	...
	// We need a moniker, so wait for it
	if (!this.moniker) {
		return this.once('got_moniker', function retry() {
...
Creature.setMethod(function getParent(type, callback) {
	...
	this.loadHistory(function gotHistory(err, history) {
		...
		that.app.getCreature(moniker, function gotParent(err, parent) {

			if (err) {
				console.error('Unable to get creature with moniker "' + moniker + '"', err);
				return callback();
			}
...
Creature.setMethod(function getGeneration(callback) {
	...
	if (!this.hasBeenSeen('ready')) {
		...
		return this.once('ready', function afterReady() {
			console.log('Ready seen, regetting generation')
			that.getGeneration(callback);
		});
	}

	if (this.hasBeenSeen('getting_generation')) {
		console.log('Alreaddy getting generation, waiting for got_generation')
	...
	Fn.series(function getFather(next) {
		that.getFather(function gotFather(err, father) {
			...
			if (err) {
				console.error('Unable to get father of', that.moniker);
				return next();
			}
		...
		that.getMother(function gotMother(err, mother) {
			...
			if (err) {
				console.error('Unable to get mother of', that.moniker);
				return next();
			}
		...
		console.log('Got parents', err, result);
 ...
 * @param    {Function}   callback
 */
Creature.setAfterMethod('ready', function getBodyPartImage(part_name, callback) {
	...
	genes = that.gene.getGenesOfName('Appearance');
	...
	s16 = new Blast.Classes.Develry.Creatures.S16(that.app, filename);

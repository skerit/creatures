...
Genome.setMethod(function getPigmentInfo(creature) {
	...
	for (i = 0; i < genes.length; i++) {
		...
		if (gene.data && gene.data.color_name) {
			switch (gene.data.color_name) {
				case 'red':
					r.push(gene.data.intensity);
					break;

				case 'green':
					g.push(gene.data.intensity);
					break;

				case 'blue':
					b.push(gene.data.intensity);
					break;
			}
		}
	}
	...
	if (r.length) {
		r =  1 + (0.25 * ((Blast.Bound.Math.mean(r) - 128) / 128));
	} else {
		r = 1;
	}

	if (g.length) {
		g =  1 + (0.25 * ((Blast.Bound.Math.mean(g) - 128) / 128));
	} else {
		g = 1;
	}

	if (b.length) {
		b =  1 + (0.25 * ((Blast.Bound.Math.mean(b) - 128) / 128));
	} else {
		b = 1;
	}
...
Genome.setMethod(function getPigmentBleedInfo(creature) {
	...
	console.log('Pigment bleed genes:', result);
...
Genome.setMethod(function getBodyPartImage(creature, part_name, callback) {
	...
	console.log('S16:', s16, this.getGenesOfName('Pigment'), this.getGenesOfName('Pigment bleed'));

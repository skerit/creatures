 ...
 * @version  0.2.2
 */
App.setMethod(function getLocale() {
	...
	// Trim the result
	this.locale = stdout.toString().trim();
...
App.setMethod(function getCreatures(update, callback) {
	...
	if (this.hasBeenSeen('getting_creatures')) {
		return this.once('_got_creatures', function gotCreatures(err, creatures) {
			callback(err, creatures);
		});
	}

...
App.setMethod(function getCreatures(update, callback) {
	...
	Fn.series(function getCreatureIds(next) {
		that.getCreatureIds(function gotIds(err, ids) {
			...
			Fn.parallel(tasks, function doneTasks(err, _result) {
		...
		if (disappearance_count > 1) {
			that.getWorldName(result, function done(err, name) {
				if (err) {

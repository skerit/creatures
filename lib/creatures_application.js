 ...
 * @param    {Function} callback
 */
App.setAfterMethod('ready', function getCreatures(update, callback, force) {
	...
	if (!force && this.hasBeenSeen('getting_creatures')) {

		// Silly, just add a timeout.
		// But sometimes getCreatures gets called recursively, so
		setTimeout(function checkAgain() {
			that.getCreatures(update, callback, true);
		}, 200);

...
SfcOle.setMethod(function _sendJSON(target, object, callback) {
	...
	callback = Fn.regulate(callback, function overflow(count, args) {
		console.log('VBOLE is too late:', count, args);
	});
	...
	this[target + '_queue'].add(function queueCommand(done) {
		...
		var bomb = Fn.timebomb(timeout, function timedout(err) {
			console.error('VBOLE "' + target + '"" did not respond to request after', timeout, 'ms', target, object);
			callback(new Error('VBOLE "' + target + '"" did not respond to request after ' + timeout + 'ms'));
			done();
		});
